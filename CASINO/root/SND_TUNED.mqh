...
input int CandleLimit = 2500;             // Candle Limit
...
void clsSND::Reactive(int bar)
{
     for (int i = ArraySize(this.LevelList)-1; i >= 0; i--)
     {
             ...
             )
           {
                ...
                {
                   if(this.LevelList[i].lowPrice < iLow(this.strSymbol,this.intPeriod,bar) &&
     ...
     for (int i = ArraySize(this.LevelList)-1; i >= 0; i--)
     {
             ...
             )
           {
                ...
                {
                   if(this.LevelList[i].highPrice > iHigh(this.strSymbol,this.intPeriod,bar) &&
    ...
    //WE ADD ANY NEW LEVEL INTO EXISTING
     ...
     if(this.strSNRType != "")
     {
        ...
        if(this.strSNRType == "Support")
        {
            ...
            level.highPrice = iHigh(this.strSymbol,this.intPeriod,bar+this.intEngulfedCandleDist);
            ...
            if(this.strSNRType == "Resistance")
              {
               ...
               level.lowPrice = iLow(this.strSymbol,this.intPeriod,bar+this.intEngulfedCandleDist);
              }
        }

      level.isActive = true;
      level.startDate = iTime(this.strSymbol,this.intPeriod,bar+this.intEngulfedCandleDist);
   ...
   else
   {
         if (type == 2)
         {
               for(int i = 0; i < ArraySize(this.FreshLevelList); i++)
               {
                  ...
                  {
                    //DO SOME COUNTING
                    if(this.FreshLevelList[i].type == "Support") this.intSupCount += 1;
                    if(this.FreshLevelList[i].type  == "Resistance") this.intResCount += 1;
                    //DRAW OPTION
...
void clsSND::FindNearestLevel(int cur_bar)
{
      ...
      if(this.LevelList[i].isActive)
      {
          ...
          if(this.LevelList[i].type == "Support")
          {
               ...
               if(//this.LevelList[i].highPrice > iClose(this.strSymbol,this.intPeriod,cur_bar) ||
                  this.LevelList[i].lowPrice < iLow(this.strSymbol,this.intPeriod,cur_bar)
                 )
               {
          ...
          else
          {
                if(this.LevelList[i].type == "Resistance")
                {
                     ...
                     if(//this.LevelList[i].lowPrice < iClose(this.strSymbol,this.intPeriod,cur_bar) ||
                        this.LevelList[i].highPrice > iHigh(this.strSymbol,this.intPeriod,cur_bar)
                       )
                     {
   ...
   /*
   if(ArraySize(this.res_up_list)>=this.intMaxStoreCount) 
   {
